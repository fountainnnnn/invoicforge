...
export default function CreateInvoice() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setInvoiceId(id)
    const inv = { id, client, email, amount: total, status: 'pending', date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) }
    const saved = JSON.parse(localStorage.getItem('invoicforge_invoices') || '[]')
    saved.unshift(inv)
    localStorage.setItem('invoicforge_invoices', JSON.stringify(saved))
    setCreated(true)
  }
  ...
  if (created) {
    return (<div className="min-h-screen bg-cream flex items-center justify-center">
      <div className="card p-12 max-w-md text-center">
        <div className="w-16 h-16 rounded-full bg-leaf/20 flex items-center justify-center mx-auto mb-6"><Send size={32} className="text-leaf" /></div>
        <h2 className="font-display text-2xl font-bold text-ink mb-3">Invoice Created!</h2>
        <p className="text-sm text-muted mb-2">{invoiceId} — ${total.toFixed(2)}</p>
        <p className="text-muted mb-8">Ready to send to {client}.</p>
        <div className="flex gap-4 justify-center">
          <Link to="/dashboard" className="btn-primary">View Dashboard</Link>
          <Link to="/create" className="btn-outline">Create Another</Link>
        </div>
      </div>
    </div>)
  }
    ...
    <div className="min-h-screen bg-cream">
      <nav className="border-b border-stone/30 bg-white/60 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-gold flex items-center justify-center"><FileText size={16} className="text-white" /></div><span className="font-display text-xl font-semibold text-ink">InvoiceForge</span></div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="card p-6 mb-6">
            ...
            <div className="grid md:grid-cols-2 gap-5">
              <div><label className="label">Client Name</label><input className="input" value={client} onChange={e => setClient(e.target.value)} placeholder="e.g. Acme Corp" required /></div>
              <div><label className="label">Client Email</label><input className="input" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="billing@acme.com" required /></div>
            </div>
          </div>
          <div className="card p-6 mb-6">
            ...
            {items.map((item, i) => (
              <div key={i} className="flex items-end gap-3 mb-4 pb-4 border-b border-stone/20 last:border-0">
                ...
                <div className="w-20 pt-6 text-right font-medium text-sm">${(item.qty * item.rate).toLocaleString()}</div>
                <button type="button" onClick={() => removeItem(i)} className="pb-1 text-muted hover:text-red-500"><Trash2 size={16} /></button>
              </div>
            ))}
            <button type="button" onClick={addItem} className="btn-outline text-sm mt-2"><Plus size={14} /> Add Line Item</button>
          </div>
          <div className="card p-6 mb-6">
            <div className="max-w-xs ml-auto space-y-2">
              ...
              <div className="flex justify-between text-sm"><span className="text-muted">Tax (9%)</span><span>${(subtotal * 0.09).toFixed(2)}</span></div>
          ...
          <button type="submit" className="btn-primary w-full justify-center text-base py-3"><Send size={16} /> Create Invoice</button>
        </form>
      </div>
    </div>
  )
